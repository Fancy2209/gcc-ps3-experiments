...
spu_option_override (void)
{
  ...
  SET_OPTION_IF_UNSET (&global_options, &global_options_set, param_max_pending_list_length, 256);

  /* Small loops will be unpeeled at -O3.  For SPU it is more important
     to keep code small by default.  */
  if (!flag_unroll_loops && !flag_peel_loops)
    SET_OPTION_IF_UNSET (&global_options, &global_options_set, param_max_completely_peel_times, 4);
...
spu_legitimate_address_p (machine_mode mode,
			  rtx x, bool reg_ok_strict, code_helper)
{
...
spu_addr_space_legitimate_address_p (machine_mode mode, rtx x,
				     bool reg_ok_strict, addr_space_t as, code_helper unused)
{
  if (as == ADDR_SPACE_EA)
  ...
  return spu_legitimate_address_p (mode, x, reg_ok_strict, unused);
}
...
spu_function_value (const_tree type, const_tree func ATTRIBUTE_UNUSED)
{
  ...
  if ((mode == BLKmode || (type && AGGREGATE_TYPE_P (type)))
      ...
      if (n < nregs)
	{
	  if (byte_size < 4)
	    byte_size = 4;
	  smode = smallest_int_mode_for_size (byte_size * BITS_PER_UNIT).require ();
	  RTVEC_ELT (v, n) =
...
spu_function_arg (cumulative_args_t cum_v, const function_arg_info &arg)
{
  ...
  /* Make sure small structs are left justified in a register. */
  if ((arg.mode == BLKmode || (arg.type && AGGREGATE_TYPE_P (arg.type)))
...
static void
spu_function_arg_advance_1 (cumulative_args_t cum_v, machine_mode mode,
			  const_tree type, bool named ATTRIBUTE_UNUSED)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);
...
void
spu_function_arg_advance (cumulative_args_t cum,
			     const function_arg_info &arg)
{
  spu_function_arg_advance_1 (cum,
			  ...
			  gimple_seq * post_p ATTRIBUTE_UNUSED)
{
  ...
  pass_by_reference_p = pass_by_reference (NULL, function_arg_info(type, false));
...
class spu_vector_costs : public vector_costs
{
public:
...
//
//static unsigned
//spu_add_stmt_cost (void *data, int count, vect_cost_for_stmt kind,
...
class spu_cost_data : public vector_costs
{
public:
  using vector_costs::vector_costs;

  unsigned add_stmt_cost (int count, vect_cost_for_stmt kind,
                          stmt_vec_info stmt_info, slp_tree, tree vectype,
                          int misalign,
                          vect_cost_model_location where) override;
...
unsigned
spu_cost_data::add_stmt_cost (int count, vect_cost_for_stmt kind,
  ...
  if (flag_vect_cost_model)
    {
	 ...
	 vectorized are weighted more heavily.  The value here is
	 arbitrary and could potentially be improved with analysis.  */
      if (where == vect_body && stmt_info && stmt_in_inner_loop_p (m_vinfo, stmt_info))
	count *= 50;  /* FIXME.  */
...
void
spu_cost_data::finish_cost(const vector_costs *scalar_costs)
{
//  *prologue_cost = cost[vect_prologue];
//  *body_cost     = cost[vect_body];
//  *epilogue_cost = cost[vect_epilogue];
...
static vector_costs *
spu_vectorize_create_costs (vec_info *vinfo, bool costing_for_scalar)
{
  return new spu_cost_data (vinfo, costing_for_scalar);
}
...
static const attribute_spec spu_gnu_attributes[] =
{
...
static const scoped_attribute_specs spu_gnu_attribute_table =
{
  "gnu", { spu_gnu_attributes }
};

static const scoped_attribute_specs *const spu_attribute_table[] =
{
  &spu_gnu_attribute_table
};
...
#define TARGET_VECTORIZE_CREATE_COSTS spu_vectorize_create_costs
